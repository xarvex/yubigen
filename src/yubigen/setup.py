...
def change_openpgp_admin_pin(connection_or_session: SmartCardConnection | OpenPgpSession):
    ...
    session.change_admin(
        prompt("Enter Admin PIN", "12345678", hide_input=True),
        prompt("New Admin PIN", hide_input=True, confirmation_prompt=True),
    )
...
def change_openpgp_pin(connection_or_session: SmartCardConnection | OpenPgpSession):
    ...
    session.change_pin(
        prompt("Enter PIN", "123456", hide_input=True),
        prompt("New PIN", hide_input=True, confirmation_prompt=True),
    )
